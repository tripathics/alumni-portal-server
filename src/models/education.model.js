...
class Educations {
  static async createOrUpdate(userId, educationData) {
    ...
    if (educationData.id && educationData.institute === NITAP) {
      const membershipForms = await MembershipApplications.find({
        'membership_applications.user_id': userId,
      });

      if (
        membershipForms.some(
          (form) => form.status === 'approved' || form.status === 'pending',
        )
      ) {
        throw new ApiError(
          403,
          'Education',
          `Updating education details at ${NITAP} is not allowed unless membership application is rejected`,
        );
      }
    }
    ...
    if (!educationData.id && educationData.institute !== NITAP) {
      ...
      if (!existingEducation.length) {
        throw new ApiError(
          400,
          'Education',
          `First education should be of ${NITAP}`,
        );
      }
    }
